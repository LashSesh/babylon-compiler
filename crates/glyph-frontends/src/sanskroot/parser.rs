//! Parser for the Sanskroot language.
...
impl Parser {
    ...
    fn parse_function(&mut self) -> Result<Function, ParseError> {
        self.expect(&Token::Keyword(KeywordKind::Karya))?;
...
mod tests {
    ...
    fn test_parse_minimal_print() {
        ...
        match &f.body[0] {
            Statement::ExprStmt(Expr::Call { callee, args }) => {
                ...
                assert_eq!(args[0], Expr::Literal(LitValue::String("नमस्ते".into())));
            }
    ...
    #[test]
    fn test_parse_multi_function() {
        let src = "कार्य योग(a, b) {\n    निवृत्ति a + b;\n}\nकार्य मुख्य() {\n    मान परिणाम = योग(3, 4);\n    दर्शय(परिणाम);\n}";
        let tokens = lex(src).unwrap();
        let program = parse(&tokens).unwrap();
        assert_eq!(program.functions.len(), 2);
        assert_eq!(program.functions[0].name, "योग");
        assert_eq!(program.functions[1].name, "मुख्य");
    }
}
