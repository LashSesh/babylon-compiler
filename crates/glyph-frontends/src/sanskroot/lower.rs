//! Lowering pass from Sanskroot AST to glyph IR.
...
/// Lower a parsed Sanskroot program into an IR document.
pub fn lower(program: &Program, source_digest: &str) -> IrDocument {
    let mut doc = IrDocument::new("sanskroot", source_digest);
...
mod tests {
    use super::*;
    use crate::sanskroot::lexer::lex;
    use crate::sanskroot::parser::parse;
    ...
    fn test_tv1_minimal_print() {
        ...
        // Should have Module, Function, Block, Call, Identifier(print), Literal
        ...
        assert_eq!(func_nodes[0].name, "मुख्य");
        ...
        assert_eq!(lit_nodes[0].value, Some(LiteralValue::String("नमस्ते".into())));
        assert_eq!(lit_nodes[0].literal_type, Some("string".into()));

        // Verify contains edges: Module->Function, Function->Block
        let contains_edges: Vec<_> = doc.edges.iter().filter(|e| e.kind == EdgeKind::Contains).collect();
        assert!(contains_edges.len() >= 2);

        // Verify callee_ref edge
